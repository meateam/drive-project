...
for service in $(cat $JSON_FILE | jq -r '.[]| @base64') ; do
    ...
    # If not, check if the tag exists on git, building an image and upload to acr
    ...
    if [ "$1" == "--helm" ] || [ "$2" == "--helm" ]; then
        echo "Check helm chart file..."
         
        # Change the helm chart tag image if not updated
        if [ -z "$(helm show values z-helm/$service_name | grep tag | grep $service_tag)" ]; then
            echo "Changing the image tag for helm chart $service_name to tag $service_tag"
            sed -r "s/^(\s*tag\s*:\s*).*/\1\"${service_tag}\"/" -i z-helm/$service_name/values.yaml 
        fi
    fi

    if [ "$1" == "--zip" ] || [ "$2" == "--zip" ]; then
        echo "zip image $service_name:$service_tag..."
